...
library UDVTType {
    ...
    uint256 constant FOO_WIDTH_BITS = 8;
    ...
    uint256 constant BAR_WIDTH_BITS = 31;
    ...
    uint256 constant BAZ_WIDTH_BITS = 69;
    ...
    uint256 constant QUX_WIDTH_BITS = 25;

    error UnsafeValue();

    function createUDVT(int256 _foo, bytes32 _bar, uint256 _baz, uint256 _qux) internal pure returns (UDVT self) {
        bool err;
        assembly {
    ...
    function unpackUDVT(UDVT self) internal pure returns (int256 _foo, bytes32 _bar, uint256 _baz, uint256 _qux) {
        assembly {
    ...
    function getQux(UDVT self) internal pure returns (uint256 _qux) {
        assembly {
            _qux := shl(QUX_EXPANSION_BITS, and(shr(QUX_OFFSET, self), _25_BIT_END_MASK))
        }
    }
    ...
    function setQux(UDVT self, uint256 _qux) internal pure returns (UDVT updated) {
        assembly {
